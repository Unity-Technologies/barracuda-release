...
struct ReadonlyTensor : Tensor
{
    ...
    {
        return Get4(b % batch, h % height, w % width, c4 % channels4);
    }
    ...
    {
        float mask0 = 4 * c4 + 0 >= channels ? 0.0 : 1.0;
        float mask1 = 4 * c4 + 1 >= channels ? 0.0 : 1.0;
        float mask2 = 4 * c4 + 2 >= channels ? 0.0 : 1.0;
        float mask3 = 4 * c4 + 3 >= channels ? 0.0 : 1.0;
        ...
        return Get4(b, pos - pad, c4) * float4(mask0, mask1, mask2, mask3);
    }
