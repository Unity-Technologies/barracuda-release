...
public class UnsafeArrayCPUOps : ReferenceCPUOps
{
    ...
    {
        return MaxPool2D(X, new[] {X.width, X.height}, new[] {1, 1}, new[] {0, 0});
    }

    public override Tensor GlobalAvgPool2D(Tensor X)
    {
        return AvgPool2D(X, new[] {X.width, X.height}, new[] {1, 1}, new[] {0, 0});
    }
    ...
    public override Tensor Conv2D(Tensor X, Tensor K, Tensor B, int[] stride, int[] pad)
    {
        ...
        return Conv2DUsingIm2ColSliced(X, K, B, stride, pad);
